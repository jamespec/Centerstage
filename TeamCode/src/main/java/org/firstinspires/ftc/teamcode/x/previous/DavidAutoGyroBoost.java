...
@Autonomous(name="DavidAutoGyroBoost")

public class DavidAutoGyroBoost extends LinearOpMode
{
...
;
        for (int i = 0; i < 1; i++) {
            ...
            pipeline.stage = AutoPipeline.Stage.YELLOW;

            moveToTarget(500, 0.3);
            ...
            pipeline.stage = AutoPipeline.Stage.RED;
    ...
    private void moveToTarget(int target, double power) {
        double P = 0.0004;
        double I = 0.000001;
        double sumError = 0;
        ...
        while(rightDrive.isBusy() ) {
            double error = pipeline.getPercentDiff();
            sumError += error;
